...
function populateQuickCarSelect() {
    ...
    select.innerHTML = '<option value="">Select a car to update status...</option>';
...
function updateDashboardStats() {
    ...
    document.getElementById('active-bookings').textContent = bookings.filter(b => b.status === 'active').length;
...
function loadDashboardCarsList() {
    ...
    if (cars.length === 0) {
        carsListContainer.innerHTML = '<tr><td colspan="3" class="text-center">No cars in inventory</td></tr>';
        return;
    }
    ...
    cars.forEach(car => {
        ...
        row.innerHTML = `
            ...
            <td>${car.brand}</td>
            <td>
                <span class="badge ${statusBadgeClass}">${car.status.charAt(0).toUpperCase() + car.status.slice(1)}</span>
            </td>
        `;
...
function setupAdminEventListeners() {
    ...
    document.addEventListener('click', function (e) {
        if (e.target.classList.contains('select-booking-btn') || e.target.closest('.select-booking-btn')) {
            const btn = e.target.classList.contains('select-booking-btn') ? e.target : e.target.closest('.select-booking-btn');
...
function updateCar() {
    if (!currentCarId) {
        alert('Please select a car to update first. Use the "Select a car" dropdown above.');
        return;
    }
...
function deleteCar() {
    if (!currentCarId) {
        alert('Please select a car to delete first. Use the "Select a car" dropdown above.');
        return;
    }

    if (!confirm('⚠️ Are you sure you want to delete this car? It will be removed from the "Available Cars for Rent" page as well.')) return;
    ...
    const bookings = getBookings();
    const hasActiveBookings = bookings.some(b => b.carId === currentCarId && b.status === 'active');
    ...
    alert(`✅ Car "${car.brand} ${car.model}" deleted successfully! It has been removed from the system.`);
}
...
function updateCarStatus() {
    ...
    if (oldStatus === status) {
        alert(`Car "${car.brand} ${car.model}" is already "${status}". No changes made.`);
        return;
    }
...
function markBookingAsReturned() {
    if (!selectedBookingId) {
        alert('Please select a booking first by clicking the checkmark icon (✓) next to it.');
        return;
    }
    ...
    alert('✅ Booking marked as returned! The car is now available on the main page.');
}
...
function viewBookingDetails(bookingId) {
    ...
    const displayStatus = (booking.status === 'completed' || booking.status === 'returned')
        ? 'Returned'
...
function highlightSelectedBooking(bookingId) {
    ...
    const selectedRow = document.querySelector(`tr[data-booking-id="${bookingId}"]`);
    if (selectedRow) {
