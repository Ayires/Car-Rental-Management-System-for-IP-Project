...
function handleSignup(event) {
    ...
    if (!isPasswordStrong(password)) {
        showSignupError('Password must be at least 8 characters long, contain both letters and numbers, and include at least one special character.');
        return;
    }
    ...
    if (userRole === 'admin') {
        // Validate admin code
        if (!adminCode) {
            showSignupError('Admin authorization code is required for admin registration.');
            return;
        }
        
        if (!validateAdminCode(adminCode)) {
            showSignupError('Invalid admin authorization code. Admin accounts can only be created by authorized personnel.');
            return;
        }
        
        // Additional password strength requirements for admin
        if (!isAdminPasswordStrong(password)) {
            showSignupError('Admin password must be at least 12 characters long, contain uppercase, lowercase, numbers, and special characters.');
            return;
        }
    }
...
function showErrorMessage(message) {
    ...
    if (errorElement) {
        errorElement.querySelector('span').textContent = message || 'Invalid credentials. Please try again.';
...
function showPrivacyPolicy(event) {
    event.preventDefault();
    alert('Privacy Policy: Your data is secure with us. We do not share your personal information with third parties without your consent.');
}
